...
def answer_one():
    ...
    n = 15
    Out=np.random.randn(4,100)*0.0
    for i,degree in enumerate([1,3,6,9]):
        poly=PolynomialFeatures(degree=degree)    
        X_train_poly=poly.fit_transform(X_train.reshape(X_train.shape[0],1))
        X_train_poly.shape        
        X_o=np.linspace(0,10,100).reshape(100,1)
        LReg=LinearRegression().fit(X_train_poly, y_train.reshape(y_train.shape[0],1))
        X_o_poly=poly.fit_transform(X_o)
        X_o_poly.shape
        Out[i,:]=LReg.predict(X_o_poly).reshape(1,100)
...
def answer_two():
    ...
    for i in np.arange(0,10,1):
        ...
        X_train,X_test,y_trail,y_test=train_test_split(X_poly,y,random_state=0)
        LReg=LinearRegression().fit(X_train,y_train)
        r2=LReg.score(X_train,y_train)
        r2_t=LReg.score(X_test,y_test)
        r2_train.append(r2)
        r2_test.append(r2_t)
...
def answer_four():
    ...
    r3=Lassoreg.score(X_test,y_test)
...
def answer_five():
    ...
    lst=sorted(list(zip(X_train2.columns,clf.feature_importances_)),key= lambda tup: -tup[1])
    lst2=zip(*lst)
    lst3=list(list(lst2)[0])
...
def answer_six():
    ...
    param_gamma=np.logspace(-4,1,6)
    train_score,test_score=validation_curve(SVC(C=1),X_subset,y_subset,scoring='accuracy',param_name='gamma',
                                            param_range=param_gamma)
